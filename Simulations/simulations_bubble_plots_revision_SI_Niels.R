...
file.maker <- function(sheet, data ){
  ...
  dciles.tab <- read_excel("PGS ranking/2. First revision to NHB/SI_sim_results_combined_bubble.xls", 
                           sheet = sheet, 
                           col_names=F)
...
sim_1_bubble_df <- file.maker("h050scatter114", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
sim_2_bubble_df <- file.maker("h050scatter714", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
sim_3_bubble_df <- file.maker("h075scatter114", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
sim_4_bubble_df <- file.maker("h075scatter714", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
sim_5_bubble_df <- file.maker("h100scatter114", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
sim_6_bubble_df <- file.maker("h100scatter714", data="PGS ranking/Analysis/Input/bubbleplot_twoPGIs.txt")
...
true.bubble <- grid.arrange(ncol=3,
                            nrow=2,
                            ...
                            left = textGrob(label=expression(paste(italic("R"[PGI]^2)," / ",italic("h"[SNP]^2),"= 0.975                            ", italic("R"[PGI]^2)," / ",italic("h"[SNP]^2),"= 0.50")), rot = 90, vjust = 0.75, just = "left", hjust = 0.45, gp = gpar(fontface = "bold", cex = 1)),
                            bottom = textGrob(label = expression(paste(italic("h"[SNP]^2),"= 0.50                                             ",italic("h"[SNP]^2),"= 0.75                                             ",italic("h"[SNP]^2),"= 1.00")), vjust = 0.25, hjust = 0.475, just = "left", gp = gpar(fontface = "bold", cex = 1)))
                            


# solution for scaling https://stackoverflow.com/questions/66429500/how-to-make-raggagg-png-device-work-with-ggsave
ggsave("scaled_bubble_simulations_revision_SI.png",
       true.bubble,
       width=1400*2.5*1.33,
       height=1150*2*1.33,
       scaling=1.75,
...
ggs <- image_read("scaled_bubble_simulations_revision_SI.png")
